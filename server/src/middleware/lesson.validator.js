...
const createLessonRules = [
    ...
    body('duration_seconds')
      .optional()
      .isInt({ min: 1 }).withMessage('duration_seconds must be a positive integer')
      .toInt(),

    body('is_preview')
      .optional()
      .isBoolean().withMessage('is_preview must be true or false')
      .toBoolean(),
...
const reorderLessonsRules = [
  body('lessons')
    .isArray({ min: 1 }).withMessage('lessons must be a non-empty array'),

  body('lessons.*.id')
    .isUUID().withMessage('Each lesson must have a valid UUID id'),

  body('lessons.*.order_index')
    .isInt({ min: 0 }).withMessage('Each lesson must have a non-negative order_index')
    .toInt(),
