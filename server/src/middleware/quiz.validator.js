...
const createQuizRules = [
    ...
    body('lesson_id')
      ...
      .isUUID().withMessage('lesson_id must be a valid UUID or null'),

    body('pass_percent')
      .optional()
      .isInt({ min: 1, max: 100 }).withMessage('pass_percent must be a number between 1 and 100')
      .toInt(),
    
    body('max_attempts')
      .optional()
      .isInt({ min: 1, max: 10 }).withMessage('max_attempts must be 1-10')
      .toInt(),
    ...
    body('questions.*.body')
      .trim().notEmpty().withMessage('Every question must have text'),
    ...
    body('questions.*.options.*.text')
      .trim().notEmpty().withMessage('Every option must have text'),
...
const submitAttemptRules = [
  body('answers')
    .isObject().withMessage('answers must be an object mapping questionId to optionId')
    .custom((answers) => {
