...
const uploadAvatar = async (req, res, next) => {
  try {
    ...
    await User.update({ avatar_url: url, updated_at: new Date() }, { where: { id: req.user.id } });
...
const uploadPdf = async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ message: 'PDF uploaded successfully', key });
  } catch (error) {
    next(error);
  }
};
...
const uploadVideo = async (req, res, next) => {
  try {
    ...
    console.log(
      `Processing video: ${req.file.originalname} (${(req.file.size / 1024 / 1024).toFixed(1)}MB)`
    );

    const { mp4Buffer, thumbnailBuffer, durationSeconds } = await processVideo(
      req.file.buffer,
...
export { uploadImage, uploadAvatar, uploadPdf, uploadVideo };
