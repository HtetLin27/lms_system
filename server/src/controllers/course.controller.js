...
const listCourses = async (req, res, next) => {
  try {
    const { page = 1, limit = 12, level, category, search, sort = 'created_at' } = req.query;
...
const updateCourse = async (req, res, next) => {
  try {
    ...
    if (course.status === 'published' && req.user.role !== 'admin') {
      return res
        .status(422)
        .json({ error: 'Cannot edit a published course. Submit for review first.' });
    }
  ...
  try {
    ...
    if (!['draft', 'rejected'].includes(course.status)) {
      return res.status(422).json({
        error: 'Only draft or rejected courses can be submitted for review',
    ...
    if (!course.lessons || course.lessons.length === 0) {
      return res
        .status(422)
        .json({ error: 'Course must have at least one lesson before submitting for review' });
    }
